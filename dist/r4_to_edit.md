...
1. 糖尿病の病因、病態生理、分類、症候と診断、急性合併症、慢性合併症を説明できる。糖尿病の治療（食事療法、運動療法、薬物治療）、低血糖症を説明できる。
...
1. 主な先天性疾患を列挙できる。
1. 新生児の生理的特徴、胎児機能不全(non-reassuring fetal status <NRFS>)、新生児仮死の病態を説明できる。
1. 新生児マススクリーニング、新生児黄疸の鑑別と治療、新生児期の呼吸障害の病因を列挙できる。
1. 正常児・低出生体重児・病児の管理の基本を説明できる。低出生体重児固有の疾患を概説できる。
1. 新生児聴覚スクリーニングを説明できる。
1. 乳幼児の生理機能の発達を説明できる。
1. 乳幼児の保育法・栄養法の基本を概説できる。
1. 乳幼児突然死症候群(sudden infant death syndrome <SIDS>)を説明できる。
1. 小児の精神運動発達及び心身相関、小児の栄養上の問題点、小児免疫発達と感染症の関係を概説できる。
1. 小児保健における予防接種の意義と内容を説明できる。
1. 成長に関わる主な異常（小児心身症を含む）を列挙できる。
1. 児童虐待を概説できる。
1. 小児の診断法と治療法における特徴を概説できる。
1. 神経発達障害群（自閉症スペクトラム症障害<ASD>、注意欠如・多動症障害<ADHD>、限局性学習障害、チック障害群）を列挙できる。
1. 思春期発現の機序と性徴を説明できる。
...
1. 老年症候群（歩行障害・転倒、認知機能障害、排泄障害、栄養障害、摂食・嚥下障害等）、フレイル、サルコペニア、ロコモティブ・シンドロームの概念、その対処法、予防が説明できる。
1. 認知症・うつ・せん妄、歩行障害・転倒の評価、鑑別診断を行い、原因に応じた治療・予防・リハビリテーションを説明できる。
1. 口腔機能低下、摂食・嚥下障害の評価、鑑別診断を行い、原因に応じた治療・リハビリテーション、予防を実施でき、栄養マネジメントを説明できる。
...
1. 植物状態と脳死の違い、内因死と外因死について違いと内容を説明でき、突然死の定義、突然死を来しうる疾患（乳幼児突然死症候群<SIDS>を含む）を説明できる。
1. 診療関連死を説明できる。
1. 死に至る身体と心の過程、その個別性と共感配慮、人生の最終段階における医療（エンド・オブ・ライフ・ケア）での患者とのコミュニケーション、頻度の高い苦痛とその対処法・ケアを説明できる。
...
1. 模擬患者の協力を得て、臨床技能（コミュニケーションスキルを含む）や医療者に求められる態度を身に付ける。

### 身体所見(第3層)

1. 基本的バイタルサイン（体温、呼吸、脈拍、血圧）の意義とモニターの方法を説明できる。
...
1. 上腕で触診、聴診法により血圧を測定できる。
1. 両側の橈骨動脈で脈拍を診察できる。
1. 呼吸数を測定し、呼吸の異常の有無を確認できる。
1. 腋窩で体温を測定できる。
1. 下肢の動脈の触診等、下腿の血圧測定（触診法）、大腿の血圧測定（聴診法）を実施できる。
...
1. 心音と心雑音の聴診ができる。
1. 背部の叩打痛を確認できる。
1. 乳房の診察を実施できる（シミュレータでも可とする）。
1. 腹部の視診、聴診ができる。
1. 区分に応じて腹部の打診、触診ができる。
1. 圧痛、腹膜刺激徴候、筋性防御の有無を判断できる。
1. 腹水の有無を判断できる。
1. 腸雑音、血管雑音の聴診ができる。
1. 直腸（前立腺を含む）指診を実施できる（シミュレータでも可とする）。
1. 意識レベルを判定できる。
...
1. 小児の成長・発達の評価に可能な範囲で参加できる。
1. 経皮的酸素飽和度を測定できる。
1. 基本的な内科的診察技能について学ぶ。
1. 基本的な小児科診察技能について学ぶ。
1. 基本的な婦人科診察技能について学ぶ。
1. 基本診療科[^table:基本診療科]において基本的な診察技能について学ぶ。
...
1. 内視鏡機器の種類と原理を説明できる。
1. 内視鏡検査法の種類を列挙し、概説できる。
1. 超音波機器の種類と原理を説明できる。
1. 超音波検査法の種類を列挙し、概説できる。
1. 超音波の生体作用と安全性を説明できる。
1. 超音波造影法を説明できる。
...
1. 動脈血ガス分析、経皮的酸素飽和度モニターの目的と適応を説明し、結果を解釈できる。
...
1. 臨床疫学的指標（感度・特異度、尤度比等）を考慮して、必要十分な検査を挙げ、症例における検査結果の臨床的意義を解釈できる。
...
1. 治療の立案・実施に可能な範囲で参加できる。
1. 内科的治療の立案・実施に可能な範囲で参加する。
1. 外科的治療の適応を知る。
1. 手術計画の立案に可能な範囲で参加する。
1. 周術期管理に可能な範囲で参加する。
1. 手術を含めた婦人科的治療に可能な範囲で参加する。
1. 健康問題に対する包括的アプローチ（複数の健康問題の相互作用等）を体験する。
1. 家族や地域といった視点をもち、心理・社会的背景により配慮した診療に可能な範囲で参加する。
1. 在宅医療を体験する。
1. 基本診療科[^table:基本診療科]の治療に可能な範囲で参加する。
...
1. 尿検査（尿沈渣を含む）を実施できる。
1. 末梢血塗抹標本を作成し、観察できる。
1. 微生物学検査（Gram染色を含む）を実施できる。
1. 12誘導心電図を記録できる。
1. 静脈採血を実施できる。
1. 動脈血採血・動脈ラインの確保を見学し、介助する。
1. 腰椎穿刺を見学し、介助する。
1. 尿検査（尿沈渣を含む）を実施できる。
1. 末梢血塗抹標本を作成し、観察できる。
1. 微生物学検査（Gram染色を含む）を実施できる。
1. 妊娠反応検査を実施できる。
1. 血液型判定を実施できる。
1. 視力、視野、聴力、平衡機能検査を実施できる。
1. 12誘導心電図を記録できる。
1. 脳波検査の記録ができる。
1. 眼球に直接触れる検査を見学し、介助する。
1. 心臓、腹部の超音波検査を実施できる。
1. 経皮的酸素飽和度を測定できる。
1. エックス線撮影、コンピュータ断層撮影<CT>、磁気共鳴画像法<MRI>、核医学検査、内視鏡検査を見学し、介助する。
1. 無菌操作を実施できる。
...
1. 無菌操作を実施できる。
1. 手術や手技のための手洗いができる。
1. 手術室におけるガウンテクニックができる。
1. 皮膚消毒ができる。
1. 外用薬の貼付・塗布ができる。
1. 気道内吸引、ネブライザーを実施できる。
1. 中心静脈カテーテル挿入を見学し、介助する。
1. 胃管の挿入と抜去ができる。
1. 尿道カテーテルの挿入と抜去を実施できる。
1. ドレーンの挿入と抜去を見学し、介助する。
1. 注射（皮内、皮下、筋肉、静脈内）を実施できる。
1. 全身麻酔、局所麻酔、輸血を見学し、介助する。
1. 眼球に直接触れる治療を見学し、介助する。
1. 手術や手技のための手洗いができる。
1. 手術室におけるガウンテクニックができる。
1. 基本的な縫合と抜糸ができる。
1. 創の消毒やガーゼ交換ができる。
1. 手術、術前・術中・術後管理を見学し、介助する。
1. 手術等の外科的治療に可能な範囲で参加する。
1. 基本的な外科的手技について学ぶ。
...
1. 緊急性の高い状況かどうかをある程度判断できるようになる。
1. 一次救命処置を実施できる。
1. 身体徴候、バイタルサインから緊急性の高い状況であることを判断できる。
1. 一次救命処置を実施できる。
1. 二次救命処置を含む緊急性の高い患者の初期対応に可能な範囲で参加する。

### 書類の作成(第3層)

1. 各種診断書・検案書・証明書の作成を見学し、介助する。
...
1. 症例を適切に要約する習慣を身に付け、状況に応じて提示できる。
